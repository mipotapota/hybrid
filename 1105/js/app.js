  ...
  showCardResult(card) {
    ...
    container.innerHTML = `
      <div class="card-reveal">
        ...
        ${!card.isNew ? `<div class="alert alert-info">중복 카드! (${dupCount}/3)</div>` : ''}
        <div class="card-image">${card.icon}</div>
        <div class="card-name">${card.name}</div>
        <div class="card-description">${card.description}</div>
        <div class="card-rarity" style="color: ${card.isLimited ? 'var(--pink-main)' : 'var(--gray-dark)'}; font-weight: bold;">
          ${card.rarity}
        </div>
        ${dupCount >= 3 ? '<div class="alert alert-success">중복 3장 달성! 뽑기권 1장으로 교환되었습니다!</div>' : ''}
      </div>
    `;
