...
```
- **create**: 로그인했고, 생성할 문서의 userId가 본인 UID인 경우
- **read**: 로그인했고, where 쿼리를 위해 `resource == null` 체크 포함
  - `resource == null`: 쿼리 실행 전 (문서가 아직 없을 때)
  - `resource.data.userId == request.auth.uid`: 본인의 즐겨찾기만 읽기
- **update/delete**: 본인의 즐겨찾기만 수정/삭제 가능
