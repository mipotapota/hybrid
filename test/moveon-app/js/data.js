    ...
    try {
        ...
        weekWorkouts.forEach(doc => {
            ...
            if (completedAt && completedAt >= weekStart) {
                weekDates.add(data.date);
            }
        });
        
        weekCount = weekDates.size; // 중복 날짜 제거 후 횟수
        
        // 주간 목표 (기본 3회, 사용자 설정이 있으면 그걸로)
        const weeklyGoal = 3; // TODO: 사용자 설정에서 가져오기
        
        // 달성률 계산 (최대 100%)
        const achievementRate = Math.min(Math.round((weekCount / weeklyGoal) * 100), 100);
        
        // 사용자 통계 업데이트
        const userRef = db.collection('users').doc(currentUser.uid);
        await userRef.update({
            totalWorkouts: firebase.firestore.FieldValue.increment(1),
            weeklyWorkouts: weekCount,
            achievementRate: achievementRate
        });
        
        console.log('✅ 운동 완료 기록됨:', workoutData.name);
        console.log(`📊 달성률: ${achievementRate}% (${weekCount}/${weeklyGoal}회)`);
