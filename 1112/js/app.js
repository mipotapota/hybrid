...
async function loadTodayWorkout() {
    ...
    try {
        ...
        const isCompleted = !todayWorkout.empty;
...
async function loadWorkoutRecords() {
    ...
    try {
        ...
        // 달력 표시
        displayCalendar(records);
        
        // 달성률 계산
        const totalDays = new Date().getDate();
        const workoutDays = new Set(records.docs.map(doc => doc.data().date)).size;
...
function displayCalendar(records) {
    ...
    // 운동한 날짜들
    const workoutDates = new Set(records.docs.map(doc => {
...
function displayRecordList(records) {
    ...
    records.docs.slice(0, 10).forEach(doc => {
        const data = doc.data();
        const date = new Date(data.completedAt.toDate());
